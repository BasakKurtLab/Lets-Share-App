...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    updateUserPosts(state, payload) {
      state.userposts.splice(payload.id_a, 1, {
        id: payload.id,
        user: payload.user,
    ...
    updatePosts(state, payload) {
      state.posts.splice(payload.id_a, 1, {
        id: payload.id,
        user: payload.user,
        content: payload.content,
        date: payload.date,
    ...
    updateFriends(state, payload) {
      state.friends.splice(payload.id_a, 1, {
        id: payload.id,
  ...
  actions: {
    async initUserPosts(context) {
      let response = await fetch(
        "http://localhost/userposts.php?token=" + document.cookie.split("=")[1]
      );
    ...
    async updateUserPosts(context) {
      ...
      let i = 0;
      for (let p in result) {
        context.commit("updateUserPosts", {
          id_a: i,
          id: result[p].id,
          content: result[p].content,
          date: result[p].date,
    ...
    async initPosts(context) {
      let response = await fetch(
        "http://localhost/posts.php?token=" + document.cookie.split("=")[1]
      );
    ...
    async initFriends(context) {
      let response = await fetch(
        "http://localhost/friends.php?token=" + document.cookie.split("=")[1]
      );
